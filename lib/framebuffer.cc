...
PixelDesignatorMap::~PixelDesignatorMap() { delete[] buffer_; }
...
public:
  SM5266RowAddressSetter(int double_rows, const HardwareMapping &h)
      : row_mask_(h.a | h.b | h.c), last_row_(-1), bk_(h.c), din_(h.b),
        dck_(h.a) {
  ...
  virtual void SetRowAddress(GPIO *io, int row) {
    ...
    io->ClearBits(
        bk_); // Disable serial input to keep unwanted bits out of the shifters
    last_row_ = row;
...
public:
  ShiftRegisterRowAddressSetter(int double_rows, const HardwareMapping &h)
      : double_rows_(double_rows), row_mask_(h.a | h.b), clock_(h.a),
        data_(h.b), last_row_(-1) {}
  virtual gpio_bits_t need_bits() const { return row_mask_; }
...
public:
  ABCShiftRegisterRowAddressSetter(int double_rows, const HardwareMapping &h)
      : double_rows_(double_rows), row_mask_(h.a | h.c), clock_(h.a),
        data_(h.c), last_row_(-1) {}
  virtual gpio_bits_t need_bits() const { return row_mask_; }
...
Framebuffer::Framebuffer(int rows, int columns, int parallel, int scan_mode,
                         const char *led_sequence, bool inverse_color,
                         PixelDesignatorMap **mapper)
    : rows_(rows), parallel_(parallel), height_(rows * parallel),
      columns_(columns), scan_mode_(scan_mode), inverse_color_(inverse_color),
  ...
  if (parallel > hardware_mapping_->max_parallel_chains) {
    fprintf(stderr,
            "The %s GPIO mapping only supports %d parallel chain%s, "
            "but %d was requested.\n",
            hardware_mapping_->name, hardware_mapping_->max_parallel_chains,
...
Framebuffer::~Framebuffer() { delete[] bitplane_buffer_; }
...
/* static */ void Framebuffer::InitHardwareMapping(const char *named_hardware) {
  ...
  if (!mapping) {
    fprintf(stderr, "There is no hardware mapping named '%s'.\nAvailable: ",
            named_hardware);
    for (HardwareMapping *it = matrix_hardware_mappings; it->name; ++it) {
      if (it != matrix_hardware_mappings)
        fprintf(stderr, ", ");
...
/* static */ void Framebuffer::InitGPIO(GPIO *io, int rows, int parallel,
  ...
  const bool is_some_adafruit_hat =
      (0 == strncmp(h.name, "adafruit-hat", strlen("adafruit-hat")));
  // Initialize outputs, make sure that all of these are supported bits.
  const gpio_bits_t result =
      io->InitOutputs(all_used_bits, is_some_adafruit_hat);
  ...
  sOutputEnablePulser = PinPulser::Create(
      io, h.output_enable, allow_hardware_pulsing, bitplane_timings);
}
...
static void InitFM6126(GPIO *io, const struct HardwareMapping &h, int columns) {
  const gpio_bits_t bits_on =
      h.p0_r1 | h.p0_g1 | h.p0_b1 | h.p0_r2 | h.p0_g2 | h.p0_b2 | h.p1_r1 |
      h.p1_g1 | h.p1_b1 | h.p1_r2 | h.p1_g2 | h.p1_b2 | h.p2_r1 | h.p2_g1 |
      h.p2_b1 | h.p2_r2 | h.p2_g2 | h.p2_b2 | h.p3_r1 | h.p3_g1 | h.p3_b1 |
      h.p3_r2 | h.p3_g2 | h.p3_b2 | h.p4_r1 | h.p4_g1 | h.p4_b1 | h.p4_r2 |
      h.p4_g2 | h.p4_b2 | h.p5_r1 | h.p5_g1 | h.p5_b1 | h.p5_r2 | h.p5_g2 |
      h.p5_b2 | h.a; // Address bit 'A' is always on.
...
/*static*/ void Framebuffer::InitializePanels(GPIO *io, const char *panel_type,
                                              int columns) {
  if (!panel_type || panel_type[0] == '\0')
    return;
...
inline void Framebuffer::MapColors(uint8_t r, uint8_t g, uint8_t b,
...
void Framebuffer::SetPixel(int x, int y, uint8_t r, uint8_t g, uint8_t b) {
  ...
  for (uint16_t mask = 1 << min_bit_plane; mask != 1 << kBitPlanes;
       mask <<= 1) {
...
void Framebuffer::SetPixels(int x, int y, int width, int height,
                            Color *colors) {
...
gpio_bits_t Framebuffer::GetGpioFromLedSequence(char col,
  ...
  if (pos == NULL)
    pos = strchr(led_sequence, tolower(col));
  if (pos == NULL) {
...
void Framebuffer::DumpToMatrix(GPIO *io, int pwm_low_bit) {
  ...
  for (uint8_t row_loop = 0; row_loop < double_rows_; ++row_loop) {
    ...
    case 1: // interlaced
      d_row = ((row_loop < half_double) ? (row_loop << 1)
